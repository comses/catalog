...
# replace hostname with the value of DBHOST and dbuser:dbpass with the appropriate values
#
# 
# USERNAME - Postgres user
# DBHOST - Host name (or IP address) of PostgreSQL server e.g localhost
# DBNAMES - List of DBNAMES for Daily/Weekly Backup e.g. "DB1 DB2 DB3"
# BACKUPDIR="/backups/postgres" - Backup directory location e.g /backups
###########
# Mail setup
...
#
#  Use a config file and run via `autopgsqlbackup.sh -c /path/to/cfg`
#
#=====================================================================

DEFAULT_CONFIG_FILE_PATH="/etc/autopgsqlbackup.conf"
...
if [ ! -z $CONFIG_FILE_PATH ]; then
  CONFIG_FILE="${CONFIG_FILE_PATH:-$DEFAULT_CONFIG_FILE_PATH}"
  echo "Running autopgsqlbackup on $CONFIG_FILE"
  source "$CONFIG_FILE"
fi
...
# - files : send log file and sql files as attachments (see docs)
# - stdout : will simply output the log to the screen if run manually.
if [ -z "$MAINCONTENT" ]; then
  MAILCONTENT="log"
fi

# Set the maximum allowed email size in k. (4000 = approx 5MB email [see docs])
if [ -z "$MAXATTSIZE" ]; then
  MAXATTSIZE="4000"
fi
