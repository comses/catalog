...
class TestModelManagers(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.author_detached = {'orcid': '1234', 'type': 'foo', 'creator_type': 'INDIVIDUAL'}
        cls.context = models.AuditCommand.objects.create(
            creator=cls.user, action=models.AuditCommand.Action.OTHER, role=models.AuditCommand.Role.SYSTEM_LOG)
    ...
    def test_author_log_create(self):
        author = models.Author.objects.log_create(audit_command=self.context, payload=self.author_detached)
        ...
        self.assertEqual(auditlog.payload, {'id': author.id})

    def test_author_log_get_or_create(self):
        author, created = models.Author.objects.log_get_or_create(
            audit_command=self.context, payload=self.author_detached)
        auditlog = models.AuditLog.objects.first()
        self.assertEqual(auditlog.table, 'citation_author')
        self.assertEqual(auditlog.action, 'DELETE')
        self.assertEqual(auditlog.payload, {'id': author.id})
        ...
        author2, created = models.Author.objects.log_get_or_create(
            audit_command=self.context, id=author.id, payload=self.author_detached)
    ...
    def test_author_log_update(self):
        ...
        models.Author.objects.log_update(audit_command=self.context, payload={'creator_type': 'GROUP'})
        auditlog = models.AuditLog.objects.first()
        self.assertEqual(auditlog.table, 'citation_author')
        self.assertEqual(auditlog.action, 'UPDATE')
        self.assertEqual(auditlog.payload['creator_type'], 'INDIVIDUAL')
