...
class LogQuerySet(models.query.QuerySet):
    def trace_delete(self, audit_command):
        with transaction.atomic():
            instances = self.all()

            auditlogs = []
            for instance in instances:
                payload = {field.column: getattr(instance, field.column)
...
# zotero publication metadata
    ...
    creators = models.ManyToManyField(Author)
...
class Raw(models.Model):
    ...
    publications = models.ManyToManyField(Publication, related_name='raw')
    container_aliases = models.ManyToManyField(ContainerAlias, related_name='raw')
    author_aliases = models.ManyToManyField(AuthorAlias, related_name='raw')
